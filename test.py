...
def test(args, eval_on):
    ...
    args.n_groups = test_loader.dataset.n_groups
    ...
    if eval_on == 'train':
        _, worst_case_acc, stats = utils.evaluate_groups(args, model, train_eval_loader, split='train')
    elif eval_on == 'val':
        _, worst_case_acc, stats = utils.evaluate_groups(args, model, val_loader, split='val')
    elif eval_on == 'test':
        if args.eval_corners_only:
            _, worst_case_acc, stats = utils.evaluate_groups(args, model, test_loader, split='test')
        else:
...
# Data args
parser.add_argument('--dataset', type=str, default='mnist', choices=['mnist','celeba','femnist', 'affnist', 'imagenet', 'rimagenet'])
...
if __name__ == "__main__":
    ...
    for i, ckpt_folder in enumerate(args.ckpt_folders):
        ...
        args.seed = i + 2 # Mainly to make sure seed for training and testing is not the same. Not critical.
