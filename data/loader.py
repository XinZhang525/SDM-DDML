...
from .rimagenet_dataset import rimageNetDataset
from . import static_mnist
from .samplers import ClusteredMixSampler, ConstantMixSampler, ConstantGroupSampler, ClusteredGroupSampler, ClusteredGroupClassSampler, ClusteredGroupClassSamplerALTER, ClusteredGroupClassSamplerFEW
...
def get_loader(dataset, sampling_type=None, batch_size=None, meta_batch_size=None,
    ...
    """
    if sampling_type == 'meta_learning_batch': # Sample support batches from multiple sub distributions
        batch_sampler = ClusteredGroupClassSampler(dataset, meta_batch_size, support_size, args.way_num, args.shot_num,
                                         uniform_over_groups=args.uniform_over_groups, args=args)
    ...
    elif sampling_type == 'meta_batch_mixtures': # Sample support batches from multiple sub distributions
...
def get_dataset(args):
    ...
    if args.dataset == 'celeba':
        ...
        train_dataset = affNISTDataset('train', args.data_dir)
        val_dataset = affNISTDataset('val', args.data_dir)
        test_dataset = affNISTDataset('test', args.data_dir)
        ...
        train_dataset = rimageNetDataset('train', args.data_dir)
        val_dataset = rimageNetDataset('val', args.data_dir)
        test_dataset = rimageNetDataset('test', args.data_dir)
