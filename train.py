...
# Experiment setting args
parser.add_argument('--way_num', type=int, default=5, help='Number of ways/classes')
...
parser.add_argument('--context_num', type=int, default=1, help='Used when using a convnet/resnet')


# Data args
parser.add_argument('--dataset', type=str, default='mnist', choices=['mnist', 'femnist', 'affnist', 'imagenet', 'rimagenet'])
...
# meta batch sampling
parser.add_argument('--sampling_type', type=str, default='meta_learning_batch',
        choices=['meta_learning_batch', 'meta_batch_mixtures', 'meta_batch_groups', 'uniform_over_groups', 'regular'],
                    help='Sampling type')
...
ckpt_dir = Path('output') / 'checkpoints' / f'{args.way_num}way{args.shot_num}shot' / f'{args.context_num}context' / f'{args.experiment_name}_{args.seed}_{datetime_now}'
args.cpath = './output/checkpoints/{}way{}shot/{}_{}_{}'.format(args.way_num, args.shot_num, args.experiment_name, args.seed, datetime_now)
args.ckpt_dir = ckpt_dir
os.makedirs(args.cpath)
args.support_size = args.way_num * args.shot_num
args.meta_batch_size = args.context_num*args.meta_batch_size
if args.dataset == 'femnist':
    args.sampling_type = 'meta_learning_batch_FEW'

if args.debug:
    tags.append('debug')

if args.log_wandb:
    wandb.init(name=f"{args.experiment_name}_context_num_{args.context_num}",
               project=f"DDML_{args.dataset}_{args.way_num}_way_{args.shot_num}",
               tags=tags)
...
def main():
    ...
    for i in range(args.context_num):
        model.context_net[i] = model.context_net[i].to(args.device)
    ...
    elif args.optimizer == 'sgd': # From DRNN paper
    ...
    for epoch in trange(args.num_epochs):
        ...
        if epoch % args.epochs_per_eval == 0:
            ...
            average_case_acc, worst_case_acc, stats, content_cache = utils.evaluate_groups(args, model, val_loader, epoch, split='val') # validation
            ...
            pickle.dump(content_cache, open(args.cpath+'/content_cache.pkl', 'wb'))
            ...
            print(f"Epoch: ", epoch, "Average Acc: ", average_case_acc)
